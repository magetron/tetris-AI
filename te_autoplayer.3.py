            ...
            for x in range(fix, MAXCOL - width + fix + 1):
                ...
                for aangle in range(0, ooptimise):
                    ...
                    for xx in range(ffix, MAXCOL - wwidth + ffix + 1):
                        ...
                        tttmp_type = cclone.get_falling_block_type();
                        if (tttmp_type == "I"):
                            oooptimise = 2;
                        elif (tttmp_type == "O"):
                            oooptimise = 1;
                            ...
                            oooptimise = 4;
                        for aaangle in range(0, oooptimise):
                            if (tttmp_type == "O"):
                                fffix = -1;
                            elif (aaangle == 1):
                                if (tttmp_type == "I"):
                            ...
                            if (tttmp_type == "I"):
                               ...
                               wwwidth = 2;
                                ...
                                if (aaangle == 0) or (aaangle == 2):
                                    ...
                                    wwwidth = 2;
                            for xxx in range(fffix, MAXCOL - wwwidth + fffix + 1):
                                #print("x = " + str(x) + "\n");
                                #clone.print_block_tiles();
                                ccclone = cclone.clone(True);
                                cccurrent_angle = ccclone.get_falling_block_angle();
                                (cccurrent_x, cccurrent_y) = ccclone.get_falling_block_position();
                                cccheck = False;
                                while (cccurrent_angle != aaangle):
                                    if (cccurrent_angle < aaangle):
                                        ccclone.rotate(Direction.RIGHT);
                                    else:
                                        ccclone.rotate(Direction.LEFT);
                                    cccheck = ccclone.update();
                                    if (cccheck == True):
                                        break;
                                    cccurrent_angle = ccclone.get_falling_block_angle();
                                cccannot_move_here = False;
                                while (cccurrent_x != xx) and (cccheck == False):
                                    ppprev_x = cccurrent_x;
                                    if (cccurrent_x > xx):
                                        ccclone.move(Direction.LEFT);
                                    elif (cccurrent_x < xx):
                                        ccclone.move(Direction.RIGHT);
                                    cccheck = ccclone.update();
                                    if (cccheck == True):
                                        break;
                                    (cccurrent_x, cccurrent_y) = ccclone.get_falling_block_position();
                                    if (ppprev_x == cccurrent_x):
                                        cccannot_move_here = True;
                                        break;
                                if (cccannot_move_here == True):
                                    continue;
                                #print("gamestate_type = " + str(tmp_type) + "\n");
                                #tmp_type = cclone.get_falling_block_type();
                                #print(str(clone.get_falling_block_type()) + " " + str(clone.get_falling_block_angle()) + " " + str(clone.get_falling_block_position()));
                                ...
                                if (xxx + 5 < MAXCOL):
                                    ...
                                    xxx_test = xxx - 5;
                                tttile_map = ccclone.get_tiles();
                                yyy = 0;
                                while (yyy < MAXROW) and (tttile_map[yyy][xxx_test] == 0):
                                    yyy += 1;
                                xxx_test_height = MAXROW - yyy;
                                #print("test height is" + str(x_test_height));
                                while (cccheck == False):
                                    cccheck = ccclone.update();
                                    if (cccheck == True):
                                ...
                                tttile_map = ccclone.get_tiles();
                                yyy = 0;
                                while (yyy < MAXROW) and (tttile_map[yyy][xx_test] == 0):
                                    yyy += 1;
                                xxx_test_new_height = MAXROW - yyy;
                                #print("new test height is" + str(x_test_new_height));
                                cccomplete_lines = xxx_test_height - xxx_test_new_height;
                                if (cccomplete_lines < 0):
                                    cccomplete_lines = 0;
                                #print("complete_lines = " + str(complete_lines) + " ccomplete_lines = " + str(ccomplete_lines) + "\n");
                                ccclone.print_tiles();
                                tttile_map = ccclone.get_tiles();

                                total_complete_lines = complete_lines + ccomplete_lines + cccomplete_lines;
